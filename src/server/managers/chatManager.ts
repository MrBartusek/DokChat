...
export default class ChatManager {
	...
	public static async chatInfo(req: Request | Handshake, conversationId: string): Promise<Chat> {
		...
		const [avatar, name] = await ChatManager.generateAvatarAndName(req, chats.rows[0].id, chats.rows[0].name, chats.rows[0].id);
		return {
			id: conversationId,
			avatar: avatar,
			name: name
	...
	public static async generateAvatarAndName(
		...
		const participants = await ChatManager.listParticipants(req, id);
		...
		if(isSelfChat) {
			...
			return [
				rawAvatar || Utils.avatarUrl(req, id),
